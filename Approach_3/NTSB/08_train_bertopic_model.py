...
MODEL_OUT = Path(
    r"C:\Users\55279\Desktop\Mestrado\0.Thesis\Codes\Approach_3\NTSB\bertopic_model.pkl"
)
DOC_TOPICS_OUT = Path(
...
BINARY_MATRIX_OUT = Path(
    r"C:\Users\55279\Desktop\Mestrado\0.Thesis\Codes\Approach_3\NTSB\binary_matrix.csv"
)
...
def main():
    ...
    df["text"] = df["text"].apply(lambda x: expand_abbreviations(x, abbreviations))
    ...
    if n_docs < MIN_DOCS_REQUIRED:
        print(f"[SKIP] Insufficient documents: {n_docs} < {MIN_DOCS_REQUIRED} required")
        return
    ...
    binary_df.to_csv(BINARY_MATRIX_OUT, index=False, encoding="utf-8")
    print(f"[OK] Binary matrix saved to: {BINARY_MATRIX_OUT}")
