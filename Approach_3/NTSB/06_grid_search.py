...
SMART_GRID_PARAMS = {
    "stage1": {
        "MIN_TOPIC_SIZE": [30, 50],
        "MIN_SAMPLE_SIZE": [8, 10],
        "NUM_NEIGHBORS": [15, 30, 45]
    },
    "stage2": {
        "MIN_TOPIC_SIZE": [],
        "MIN_SAMPLE_SIZE": [],
...
def train_bertopic_with_params(params, docs, embedding_model):
    ...
    try:
        ...
        # Early pruning
        if coherence < 0.55:
            print(f"-> Pruned (score {coherence:.4f} < threshold 0.55)")
            return None
...
def generate_fine_search_params(stage1_results, max_searches=12):
    ...
    param_bounds = {
        "MIN_TOPIC_SIZE": (20, 60),
        "MIN_SAMPLE_SIZE": (5, 15),
        "NUM_NEIGHBORS": (10, 60)
    }
...
def run_fine_stage_search(stage1_results, docs, embedding_model,
    ...
    fine_params_list = generate_fine_search_params(
        stage1_results, max_searches=12)
...
def main():
    ...
    if "text" not in df.columns:
        print("[SKIP] topic_docs.csv missing required 'text' column.")
        return
